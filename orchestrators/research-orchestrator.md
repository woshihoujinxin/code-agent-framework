...
**用户只需要**：
- ✅ 在项目目录中运行：`/goal-research v0.0.1 "项目目标描述"`
- ✅ 等待自动完成

**用户不需要**：
- ❌ 手动创建 requirements.md 或任何其他文档
- ❌ 手动初始化 git 仓库或执行 git commit
- ❌ 手动执行任何衔接命令（如 /goal-review 或 /goal-develop）
- ❌ 手动创建目录结构

**orchestrator 会自动完成**：
- 自动创建项目结构（docs/、tests/、src/ 等）
- 自动初始化 git 仓库并打 tag
- 自动生成所有文档（research.md、requirement.md、prd.md 等）
- 自动衔接评审流程
- 自动衔接开发流程
- 整个流程一键完成
