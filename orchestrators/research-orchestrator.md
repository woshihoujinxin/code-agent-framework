...
### 这次调研是干嘛的
你要调研「{调研目标}」。我会做这几件事：
1. 把参考仓库下载到 references/（已加 .gitignore，不污染你的 git 历史）
2. 让调研工程师分析它们的架构，产出「技术方案图」docs/research-tech-{RSTAMP}.md
   （架构图 / 实体关系 / 状态 / 时序图——给架构师写设计文档当基准）
3. 顺带产出一份「精简需求表」docs/requirement-{RSTAMP}.md（给产品经理写 PRD 当基准）
4. 完成后自动进入评审会议（/goal-review），过完评审才开发——
...
| 术语 | 人话解释 |
|------|---------|
| RSTAMP | 本次调研的编号（`年月日-时分`），所有产出文件都带它；多次调研按编号区分先后 |
...
────────────────────────────
✅ 调研完成（批次 {RSTAMP}）——参考 {N} 个开源项目
一句话发现：{如：主流做法是 A 架构 + B 状态管理；或：3 个项目做法差异大，需评审定夺}

产出物（2 份，供人看）：
- 技术方案图：docs/research-tech-{RSTAMP}.md（架构/实体/状态/时序图）
- 精简需求表：docs/requirement-{RSTAMP}.md
