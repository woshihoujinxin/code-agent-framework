...
│  👤 用户（随时追加 / 修正需求）                                     │
│    │ req                                                            │
│    ▼  路由分级：标准SOP（默认）/ 快速模式 / BugFix                   │
│                                                                     │
│  ╭─ Phase 0 需求分析 ───────────────────────────────────────────╮  │
│  │ 调研 A4（复杂需求可选）：开源代码 → research-tech + requirement │  │
│  │ PM → prd.md（需求池 + 用户故事 + 视觉意图）                    │  │
│  │ 原型团队 A3（Web 需求）：发现5维 → 构建(71套选型) → 独立审查 → 导出 │  │
│  │           → docs/prototype/DESIGN.md（视觉基准）               │  │
│  ╰────────────────────────────────────────────────────────────────╯  │
│    ▼                                                               │
│  ╭─ Phase 1 计划 ──────────────────────────────────────────────╮  │
│  │ 架构师 → dev-plan.md + feature-spec.md（契约 F/B/S/E/Q）+ design.md │  │
│  ╰────────────────────────────────────────────────────────────────╯  │
│    ▼                                                               │
│  ╭─ 中层① 开发循环（Phase 2，全量开发，不穿插测试）─────────────╮  │
│  │ 取就绪 ⏳ → 开发批次(全部并行) → ◆冒烟(单测+自检) → 标 🔳          │  │
│  │                        ↺ 直到无 ⏳（整版本开发完）              │  │
│  ╰────────────────────────────────────────────────────────────────╯  │
│    ▼ 整版本提测                                                    │
│  ╭─ 中层② 测试循环（Phase 3）──────────────────────────────────╮  │
│  │ worktree 就绪核验 → 全量五维（🔳 × 功能/质量/健壮/安全/E2E）     │  │
│  │        → results.json（机器真源）                              │  │
│  │   ┌─ 内层 修正循环(≤3轮) ───────────────┐                     │  │
│  │   │ FAIL→resume Dev→同步worktree→重测    │                     │  │
│  │   └──────────────────────────────────────┘                     │  │
│  │   全PASS→✅ / 3轮仍FAIL→升级（PM+架构师重拆→新⏳）               │  │
│  ╰────────────────────────────────────────────────────────────────╯  │
│    │ 升级新⏳ ────────────────────────────────────────► 回中层①   │
│    ▼                                                               │
│  ╭─ Phase 4 收尾 ──────────────────────────────────────────────╮  │
│  │ SUMMARY-{version}.md / metrics.md / 运行指南 → 🏁 版本完成      │  │
│  │ （不退出循环，进入等待，检查 prd.md 是否有新需求）              │  │
│  ╰────────────────────────────────────────────────────────────────╯  │
│    ▼                                                               │
│  code-sage ◄── 所有报告 + metrics：提炼高频标签为防错规则             │
│    ▼                                                               │
...
| **自进化**（跨需求） | 收尾 / 每 5 开发批次 | code-sage 提炼标签 → coding-standards → 下次全员读，同类错误递减 |
...
> **存量项目**（有源文件 + 已有 git/依赖/测试）：走「**存量模式**（修旧如旧）」——先产 `docs/project-profile.md` 摸清存量开发方式（架构/测试/命名/错误处理），**照存量风格改**，不套 DDD/五维/三层循环；验证用项目已有测试命令；小改动走 BugFix/快速，大功能按存量适配。
...
- 每议题 ≤2 轮、整场 ≤3 个阶段，阶段3后未决项强制延期，禁止无限开会
...
1. 创建 `.claude/skills/coding-standards/references/coding-rules.md`，写入实际规范
