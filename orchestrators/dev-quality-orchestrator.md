...
5. 确认任务波宽，记为 `BATCH_SIZE`（开发阶段同层并发任务数，**默认最大化 = 取全部就绪任务**；想限流可显式设 `=1`/`=2`/`=3`，`=1` 则按依赖顺序逐个推进；429 自动降档兜底）
...
|----|------|--------|------|-----------|
| ① 任务级（开发） | `BATCH_SIZE`（任务波宽） | 一次并发几个**就绪任务**开发 | **最大化（取全部就绪）** | 2 × 就绪任务数（每任务 FE+BE） |
...
**`BATCH_SIZE`（任务波宽，开发阶段）**：开发波**取全部就绪任务**（ready_dev 全量入波，最大化并发）——master 主动开足 agent，**不等用户提醒拆分**。**异构优先**（全栈项目波内混 FE+BE，发挥前后端并行，避免同类型扎堆争抢 DB/API）。
...
**退避重试**（区分失败类型；重试的是同一调度动作、不污染业务状态）：
...
- **恢复续跑**：用户（几小时后）说"继续" → 走「压缩后恢复机制」按 dev-plan 状态精确续（韧性第1层：⏳开发 / 🔄重做 / 🔳续测）。
...
4. ready_dev 为空时：
   ...
   - 全部 ✅/⚠️ → 直接进 Phase 4 收尾
5. 开发波 = **全部 ready_dev（最大化并发，主动开足 agent，不等用户提醒拆分）**；仅当用户显式设 BATCH_SIZE 或 eff_BATCH 因 429 降档时，才取 min(eff_BATCH, |ready_dev|) 个。**异构优先**（全栈项目同波尽量混 FE+BE：若就绪集同时含 BE 与 FE 任务，优先各取一个，而非同类型扎堆），避免两个 BE 同时连 DB/写 API 的资源争抢。BATCH_SIZE=任务波宽（见「并发度控制」）。
```
...
**波内每个就绪任务各派一组 FE+BE，全部后台并行**（任务级并发 × 前后端并发）。**主动最大化并行（硬行为，不等用户提醒）**：① 就绪任务**全部**入波并行开发（不因"够用"少开 agent）；② 波内任务全 BE（如 T01 基础设施）时 master 也要**主动找前端可并行工作**（骨架完善/路由/API client mock/布局/状态管理）派 FE Dev 并行；波内任务全 FE 时同理派 BE。最大化前后端共同开发，不要等用户提示才补另一端：
