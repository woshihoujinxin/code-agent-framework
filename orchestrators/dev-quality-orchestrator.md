...
**用户只需要**：
- ✅ 在项目目录中运行：`/goal-develop <需求文档路径> <项目目录>` 或从 `/goal-research` 自动衔接
- ✅ 等待自动完成

**用户不需要**：
- ❌ 手动创建 dev-plan.md、feature-spec.md 或任何其他文档
- ❌ 手动创建项目结构（src/、tests/ 等）
- ❌ 手动执行 git commit
- ❌ 手动启动任何 agent 或测试流程

**orchestrator 会自动完成**：
- 自动创建项目结构（docs/、src/、tests/reports/ 等）
- 自动调用 Planner 生成 dev-plan.md 和 feature-spec.md
- 自动调用 Dev 并发开发（按星型依赖结构）
- 自动调用 Tester 执行 5 维测试
- 自动处理修正循环（≤3 轮）
- 自动打 tag 和归档
- 整个开发流程一键完成
...
5. 确认批次大小，记为 `BATCH_SIZE`（开发阶段同层并发任务数）：
   - **默认 = 最大化**（取全部就绪任务，用户未提供时自动设为 `'最大化'`，不为数字）
   - 想限流可显式设 `=1`/`=2`/`=3`，`=1` 则按依赖顺序逐个推进
   - **429 自动降档兜底**：遇限流由「并发自适应」把 eff_BATCH 降下来慢跑（见下）
   - **日志记录**：在速览行和 main-log 中明确记录 `BATCH_SIZE：{默认最大化/具体数字}`
...
- **主动行为**：用户未显式设置时，orchestrator 必须**默认使用最大化**，不询问用户；
...
3. 就绪集（开发优先，**只取 ready_dev**——测试延后到 Phase 3，本阶段不处理 🔳）：
   ...
   - **【调试日志】**：向用户报告"本批就绪任务：{TASK_IDs}（共 {N} 个）"
4. ready_dev 为空时：
   - 仍有 ⏳ → 上游依赖未完成，等待不启新批
