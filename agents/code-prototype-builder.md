...
> 🎯 **设计重点**：解决「原型是视觉基准，但常与最终实现脱节/自审自批」——见工作要点与负面围栏。
> 自省审：原型交互可跑吗（selftest/自检）？令牌与设计系统一致吗？关键交互（权限/中断/降级）演示全了吗？
...
- **prd.md**（调研阶段 Phase 2 产出）包含「视觉意图」段（场景/受众/调性/品牌/规模）+ 用户故事
- ⚠️ 原型阶段**必须等待 prd.md 产出后**才能执行（串行关系）
