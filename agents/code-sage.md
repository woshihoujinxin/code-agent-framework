...
3. **闭环② 指标→调优**：向 metrics.md 追加「失败模式 Top-5」（标签频次降序前 5）+「调优建议」（针对最高失败维度 1-3 条可执行建议：如"健壮性 40% → Dev 必读清单加空值守卫自查"；"某维度多次 FAIL 但契约无该类用例 → 契约缺类，建议架构师补用例"；"Dev 标 ✅ Tester 判 ❌ 比例高 → 自检可信度低，加强 Dev 自查纪律"）
