...
- 防错规则 → 写入 `coding-standards/references/contract-shared.md` 的「自进化规则」段（追加，不覆盖）
...
五个测试维度的全部标签（tester 只写自己维度的，你统计全部）：
...
4. **contract-shared.md**（`coding-standards/references/`）— 读现有「自进化规则」段，避免重复追加（自进化规则现归此文件）
...
对达到阈值的标签，生成防错规则，追加到 `coding-standards/references/contract-shared.md` 的「## 自进化规则」段：
...
**去重**：若 contract-shared.md 已存在相同标签的规则，更新其次数和来源，不重复追加。
...
读 lessons-learned.md「架构级经验」段，若发现多条同源根因（如多个任务都因"模块边界不清"失败），合并为更高阶规则追加到 contract-shared.md「自进化规则」段，标注 `[架构]`。
...
3. **保守追加**——宁缺毋滥，噪音标签丢弃，避免污染 contract-shared.md
4. **去重**——已存在的规则更新而非重复
...
- ✅ 写 coding-standards/references/contract-shared.md 的「自进化规则」段
