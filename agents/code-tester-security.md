...
> 🎯 **设计重点**：解决「开发者顺着功能写码，看不见攻击路径」——以攻击者逆向思维，画攻击面（入口→sink→数据流）找可利用点，不只查契约 S 用例。
> 自省审：7 维攻击面（注入/认证/访问/敏感数据/配置/依赖/密码学）都盘了吗？契约外新漏洞挖了吗？高危判定准吗？
...
- 明细表：
  - `## 契约 S 用例验证`（用例|测试点|攻击输入|预期|实际|判定|结果说明）
  - `## 契约外新发现漏洞`（#|维度|严重度|位置|漏洞|攻击路径|修复建议——**仅发现时写**）
- FAIL 返回附高危/中危数量
...
- **专属**：不查崩溃类（空指针/异常 → robustness）；不做功能验证（→ correctness）
