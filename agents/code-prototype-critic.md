...
### 2c. 功能可达性（Web：每个交互都必须真能点——P0 门控）

逐个审查主要交互元素，**存在任一"死交互" → FAIL**：
- 每个 `<button>` / CTA / 主链接：有 JS 响应（onclick/addEventListener），触发视图切换或状态变化
- 导航 / tab / 返回：点击实际切换对应视图（不能是装饰性 `href="#"`）
- 表单控件：输入/提交有反馈（校验/跳转）
- 多屏流程：能从一个入口点着走到终点（如 首页→详情→提交→成功）
...
- [ ] 完全无响应式
- [ ] 明显破碎布局 / 溢出
- [ ] **死交互元素**（按钮/链接/导航点击无响应或空操作，见「2c 功能可达性」）
