...
3. **docs/design.md**（或 architecture.md，若存在）— 技术决策记录 + 实体级设计 + 时序图 + 共享知识（**接口签名/组件签名的权威来源，翻译式实现**）
4. **docs/prd.md** 相关用户故事 + UI 设计稿描述 — 理解产品意图，不只看规格
...
### 4. 五维自查（对照测试契约 + 五维验收标准）
> 按 `coding-standards/references/test-acceptance-standards.md` 的"给 Dev 的执行要点"清单自查（Q 命名/函数/重复、B 空值/边界/异常/资源、S 注入/密钥/越权、视觉 Anti-Slop）。
- **功能(F)**：归属 FE 的 F 用例是否都有单测且通过
- **健壮(B)**：边界输入（空表单、超长输入）是否处理，且与 Tester 的 B 验收维度一致
- **安全(S)**：XSS、敏感数据泄露是否防护，且与 Tester 的 S 验收维度一致
